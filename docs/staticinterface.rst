...
Static Interface
================
A static interface allows a user to easily create an :class:`AddressRecord` record,
:class:`PTR` record, and generate a ``host`` statement in the DHCP configs.

.. figure:: images/static_interface.png
    :width: 100%

A static interface relies on having a ``hostname``, ``ip`` and a ``mac`` to
generate DNS and DHCP entries. The DNS entries are an A/AAAA record and a PTR record::
...
By choosing an ``ip`` you are in effect assigning the interface to an IP :class:`Range`.
...
with a :class:`StaticInterface` (SI) without causing duplicate DNS data. They are usually
...
mac address set to something like 'virtual' or 'bonded' and wouldn't be used
for anything. Two seperate BI objects would represent ``eth0`` and ``eth1`` and
would point to ``bond0``. The BI's would be the source of the two different mac
...
Interface Names
===============
On it's own, a :class:`StaticInterface` supports grouping one mac address with
it's ``A/AAAA`` and ``PTR``. In this case the interface will have a name like
``nic0``. In general, when a :class:`StaticInterface` has a single mac address
associated with it (and `no` :class:`BondedInterfaces`, it has a name like
``nic\d+``).

When a :class:`StaticInterface` needs to associate multiple mac addresses with
it's ``A/AAAA`` and ``PTR``, it uses :class:`BondedInterfaces` to add the
additional information. A :class:`StaticInterface` with
:class:`BondedInterfaces` will have a name like ``bond0`` and it's bonded
objects will have names like ``nic0.0`` and ``nic0.1``. In general, the numeral
at the end of a :class:`StaticInterface's` name will be the first number in the
name of it's bonded interfaces.

The names of both :class:`StaticInterfaces` and :class:`BondedInterfaces` are
unique within the set of interfaces associated with a system.
