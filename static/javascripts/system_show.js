...
$(document).ready(function() {
    ...
    $('.add_new_sreg').click(function(){
        ...
        $.ajax({
                ...
                dataType: 'json',
                beforeSend: function(){
                        $('#id-sreg-range').get(0).options[0] = new Option('Loading...', '');
                },
                success: function(msg) {
                        $('#id-sreg-range').get(0).options.length = 0;
                        $('#id-sreg-range').get(0).options[0] = new Option('Select Range', '');
                        $.each(msg, function(index, item) {
                            ...
                            $('#id-sreg-range').get(0).options[$('#id-sreg-range').get(0).options.length] = option
                        });
        ...
        var d = $( '#add-sreg-dialog' ).dialog({
                ...
                buttons: {
                    'Save': function() {
                            if(!$('#id_sreg-ip_str').val() && !$('#id-sreg-range').val() && $('#id-auto-assign-ip').is(':checked')){
                                         ...
                                         */
                                            ...
                                            for (var i=0; i < result.errors.hw_adapters.length; i++) {
                                                ...
                                                for (var j=0; j < result.errors.hw_adapters[i].length; j++) {
                                                    ...
                                                    error_el = $(error_list).append('<li>' + message + '</li>');
    ...
    $('#id-sreg-range').change(function(){
        ...
        if ($('#id-override-fqdn').attr('checked') === false) {
            var suggested_fqdn = $('#id-sreg-range option:selected').attr('suggested_fqdn');
    ...
    $('#btnMore').click(function() {
        ...
        nextFree = nextFree + 1;
        console.log('Next free: ' + nextFree);
    ...
    // Create/Update Hardware Adapters
